...
public class Monster {
    ...
    public void draw(Canvas canvas) {
        ...
        switch (type) {
            case TYPE_BOMB:
                // 死亡的怪物用死亡图片
                drawAni(canvas, isDie ? ViewManager.bomb2Image
                        : ViewManager.bombImage);
                break;
            case TYPE_FLY:
                // 死亡的怪物用死亡图片
                drawAni(canvas, isDie ? ViewManager.flyDieImage
                        : ViewManager.flyImage);
                break;
            case TYPE_MAN:
                // 死亡的怪物用死亡图片
                drawAni(canvas, isDie ? ViewManager.manDieImage
                        : ViewManager.manImgae);
                break;
            default:
                break;
        }
    ...
    public void drawBullet(Canvas canvas) {
        ...
        for (int i = 0; i < bulletList.size(); i++) {
            ...
            Graphics.drawMatrixImage(canvas, bitmap, 0, 0, bitmap.getWidth(),
                    bitmap.getHeight(),
                    bullet.getDir() == Player.DIR_RIGHT ? Graphics.TRANS_MIRROR
