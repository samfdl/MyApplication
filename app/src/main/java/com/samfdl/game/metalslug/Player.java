...
public class Player {
    ...
    public void draw(Canvas canvas) {
        ...
        switch (action) {
            case ACTION_STAND_RIGHT:
                drawAni(canvas, ViewManager.legStandImage,
                        ViewManager.headStandImage, DIR_RIGHT);
                break;
            case ACTION_STAND_LEFT:
                drawAni(canvas, ViewManager.legStandImage,
                        ViewManager.headStandImage, DIR_LEFT);
                break;
            case ACTION_RUN_RIGHT:
                drawAni(canvas, ViewManager.legRunImage, ViewManager.headRunImage,
                        DIR_RIGHT);
                break;
            ...
            case ACTION_JUMP_RIGHT:
                drawAni(canvas, ViewManager.legJumpImage,
                        ViewManager.headJumpImage, DIR_RIGHT);
                break;
            case ACTION_JUMP_LEFT:
                drawAni(canvas, ViewManager.legJumpImage,
                        ViewManager.headJumpImage, DIR_LEFT);
                break;
            default:
                break;
        }
    ...
    // 绘制角色的动画帧
    public void drawAni(Canvas canvas, Bitmap[] legArr, Bitmap[] headArrFrom,
                        int dir) {
        if (canvas == null) {
            return;
        }
        ...
        // 是否需要翻转图片
        int trans = dir == DIR_RIGHT ? Graphics.TRANS_MIRROR
                : Graphics.TRANS_NONE;
    ...
    public void drawHead(Canvas canvas) {
        ...
        Graphics.drawBorderString(canvas, 0xa33e11, 0xffde00, name,
                ViewManager.head.getWidth(), (int) (ViewManager.scale * 20), 3,
                p);
        // 画生命值
        Graphics.drawBorderString(canvas, 0x066a14, 0x91ff1d, "HP: " + hp,
                ViewManager.head.getWidth(), (int) (ViewManager.scale * 40), 3,
                p);
    }
    ...
    public void drawBullet(Canvas canvas) {
        ...
        for (int i = 0; i < bulletList.size(); i++) {
            ...
            Graphics.drawMatrixImage(canvas, bitmap, 0, 0, bitmap.getWidth(),
                    bitmap.getHeight(),
                    bullet.getDir() == Player.DIR_LEFT ? Graphics.TRANS_MIRROR
                            : Graphics.TRANS_NONE, bullet.getX(),
                    bullet.getY(), 0, Graphics.TIMES_SCALE);
        }
    }
    ...
    public void addBullet() {
        // 计算子弹的初始X坐标
        int bulletX = getDir() == DIR_RIGHT ? X_DEFAULT
                + (int) (ViewManager.scale * 50) : X_DEFAULT
                - (int) (ViewManager.scale * 50);
        // 创建子弹对象
        Bullet bullet = new Bullet(Bullet.BULLET_TYPE_1, bulletX, y
                - (int) (ViewManager.scale * 60), getDir());
        // 将子弹添加到用户发射的子弹集合中
