...
public class Graphics {
    ...
    // 定义绘制矩形的方法
    public static void drawRect(Canvas canvas, float x, float y, float w,
                                float h, Paint paint) {
        paint.setStyle(Style.STROKE);
    ...
    // 定义绘制弧的方法
    public static void drawArc(Canvas canvas, float x, float y, float width,
                               float height, float startAngle, float sweepAngle, Paint paint) {
        paint.setStyle(Style.STROKE);
    ...
    public static void drawString(Canvas canvas, String text, float textSize,
                                  float x, float y, int anchor, Paint paint) {
        // 动态计算字符串的对齐方式
        if ((anchor & LEFT) != 0) {
    ...
    public static void drawBorderString(Canvas c, int borderColor,
                                        int textColor, String text, int x, int y, int borderWidth,
                                        Paint mPaint) {
    ...
    public synchronized static void drawMatrixImage(Canvas canvas, Bitmap src,
        ...
        if (rotate == 0 && degree == 0 && scaleX == TIMES_SCALE) { // 即scale=1无缩放，
            // rotate=0无旋转
    ...
    public static Bitmap scale(Bitmap img, float newWidth, float newHeight) {
        ...
        try {
            // 生成对img缩放之后的图片
            return Bitmap.createBitmap(img, 0, 0, (int) width, (int) height,
                    matrix, true);
        } catch (Exception e) {
            return null;
        }
